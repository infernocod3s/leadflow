"""CLI commands for LeadFlow — argparse-based interface."""
...
from leadflow.config import CampaignConfig, get_config
from leadflow.utils.logger import console, get_logger, setup_logging
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="leadflow",
        description="LeadFlow — CLI lead enrichment pipeline",
    )
...
def cmd_import(args: argparse.Namespace) -> None:
    ...
    from leadflow.integrations.csv_handler import import_csv
...
async def cmd_run(args: argparse.Namespace) -> None:
    ...
    from leadflow.pipeline.runner import run_pipeline
...
async def cmd_push(args: argparse.Namespace) -> None:
    from leadflow.db import queries as db
    from leadflow.integrations.smartlead import push_leads_to_smartlead
...
def cmd_export(args: argparse.Namespace) -> None:
    from leadflow.db import queries as db
    from leadflow.integrations.csv_handler import export_csv
...
def cmd_steps() -> None:
    # Import to register
    import leadflow.enrichments  # noqa: F401
    from leadflow.pipeline.registry import PIPELINE_ORDER, _registry
    ...
    from leadflow.integrations.deepline import is_deepline_installed
